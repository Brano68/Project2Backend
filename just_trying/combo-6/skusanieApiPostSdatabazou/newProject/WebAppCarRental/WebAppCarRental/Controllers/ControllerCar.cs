            ...
            */
            ...
            if(list.Count == 0)
            {
                foreach (var row in contosoCarReservationContext.Cars)
                {
                    ...
                    AvailableCar availableCar = new AvailableCar(row.BrandOfCar, row.Model, row.Plate, row.Price, row.Fuel, row.Consumption, row.NumberOfPassenger, row.Gear, row.AirConditioner);
