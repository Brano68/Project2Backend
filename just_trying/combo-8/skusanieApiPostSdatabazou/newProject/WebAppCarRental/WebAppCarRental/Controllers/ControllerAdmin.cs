...
namespace WebAppCarRental.Controllers
{
    ...
    public class ControllerAdmin : ControllerBase
    {
        ...
        //metoda na vytvorenie 1. admina ak este neexistuje
        //zatial iba ako pomocna metoda
        [AllowAnonymous]
        [HttpPost]
        [Route("createfirst")]
        public async Task<ActionResult<AdminDTO>> PostAdminFirst([FromBody] FirstAdminDTO firstAdminDTO)
        {
            string login = firstAdminDTO.Login;
            string password = firstAdminDTO.Password;
            string email = firstAdminDTO.Email;
            string globalPassword = firstAdminDTO.GlobalPassword;
            ...
            string globalPasswordAdmin = System.IO.File.ReadAllText(@"AdminPassword/PasswordForAdmin.txt");
        ...
        public async Task<ActionResult<CarAddDTO>> PostAddCar([FromBody] CarAddDTO carAddDTO)
        {
            ...
            //po update databazy
            string fuel = carAddDTO.Fuel;
            double consumption = carAddDTO.Consumption;
            int numberOfPassenger = carAddDTO.NumberOfPassenger;
            string gear = carAddDTO.Gear;
            bool airConditioner = carAddDTO.AirConditioner;
            //po dalsom update
            string path = carAddDTO.Path;

            //overime ci prisli vsetke udaje
            if(login == null || model == null || brandOfCar == null || plate == null || path == null ||price == 0
